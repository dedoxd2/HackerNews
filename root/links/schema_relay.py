...
class RelayCreateLink(graphene.relay.ClientIDMutation):
    ...
    def mutate_and_get_payload(root, info, **input):
        ...
        link = Link(url=input.get('url'), description=input.get(
            'description'), posted_by=user)
        link.save()
        return RelayCreateLink(link=link)


# Note that : graphene.AbstractType doesn't exist anymore in v3.3 or 3.2
# so   you have to use graphene.ObjectType
